...
def admin_order_product_return_view(request):
    ...
    if query:  # if there is search query
        order_items = OrderProduct.objects.filter(
            Q(order_item_status="Returned") | Q(order_item_status="Return Requested")
        ).filter(product__product_name__name__icontains=query)
    else:
        order_items = OrderProduct.objects.filter(
            Q(order_item_status="Returned") | Q(order_item_status="Return Requested")
        ).order_by("-id")
    ...
    return render(
        request, "admin/admin_order_product_return.html", {"order_items": order_items}
    )
...
def admin_order_product_return_approve_view(request, order_item_id):
    ...
    try:
        ...
        with transaction.atomic():
            ...
            messages.success(
                request, "Item Return approved and amount refunded to user wallet."
            )
        ...
        messages.error(
            request,
            "An error occurred while approving return of order. Please try again.",
        )
    ...
    if request.method == "POST":
        ...
        if payment_method == "wallet":
            ...
            if wallet.balance > 0:
                ...
                amount = int(
                    online_payment_amount * 100
                )  # for converting amount to paisa for collection
...
def add_address_order_view(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            return redirect(
                "checkout_page"
            )  # Redirect to the cart page after adding the address
    else:
        form = AddAddressForm()
        ...
        if order.payment_method == "Online" or order.payment_method == "Wallet":
            ...
            try:
                ...
                with transaction.atomic():
                    ...
                    messages.success(
                        request,
                        "Cancellation of order successful and amount refunded to your wallet.",
                    )
...
def download_invoice_pdf_view(request, order_id):
    ...
    html_content = render_to_string(
        "user_home/order_invoice_pdf_download.html", {"order": order}
    )
    ...
    if request.method == "POST":
        ...
        try:
            ...
            with transaction.atomic():
                ...
                messages.success(
                    request,
                    "Order Return Succesful and amount refunded to your wallet.",
                )

        except Exception as e:
            messages.error(
                request,
                "An error occurred while returning your order. Please try again.",
            )
...
def user_cancel_order_item_view(request, order_number, item_id):
        ...
        if order.payment_method in ["Online", "Wallet", "Wallet with Online Payment"]:
            ...
            try:
                ...
                with transaction.atomic():
                    ...
                    messages.success(
                        request,
                        "Cancellation of Item successful and amount refunded to your wallet.",
                    )
    ...
    return render(
        request,
        "user_home/confirm_cancel_order_item.html",
