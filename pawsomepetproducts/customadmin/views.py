...
from django.db.models.functions import (ExtractDay, ExtractMonth, ExtractWeek,
                                        TruncDate, TruncMonth, TruncWeek)
        ...
        if form.is_valid():
            ...
            if user is not None:
                if user.is_superadmin:
                    ...
                    messages.error(
                        request, "You are not authorised to access the details"
                    )
            else:
...
def admin_dashboard_view(request):
    ...
    users_count = CustomUser.objects.filter(
        is_active=True, is_blocked=False, is_superadmin=False
    ).count()

    processing_orders_count = Order.objects.filter(
        status="Processing", is_ordered=True
    ).count()

    shipped_orders_count = Order.objects.filter(
        status="Shipped", is_ordered=True
    ).count()

    cancelled_orders_count = Order.objects.filter(
        status="Cancelled", is_ordered=True
    ).count()
...
def admin_sales_report_view(request):
    ...
    if filter_option == "daily":
        ...
        orders = orders.filter(created_at__range=[start_date, end_date]).order_by(
            "-created_at"
        )
        cancelled_orders = cancelled_orders.filter(
...
def get_category_sales_data(request):
    ...
    labels = [
        entry["product__product_name__category__name"] for entry in category_sales
    ]
