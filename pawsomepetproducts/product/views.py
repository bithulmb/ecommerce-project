...
from .forms import (AddProductForm, AddProductImages, AddProductVariantForm,
                    ReviewForm)
...
def admin_product_variants_view(request):
    ...
    return render(
        request,
        "admin/admin_product_variants.html",
...
def admin_add_edit_product_images_view(request, variant_id):
    ...
    return render(
        request,
        "admin/add_edit_product_images.html",
...
def all_products_view(request):
    ...
    if request.headers.get("x-requested-with") == "XMLHttpRequest":
        html = render_to_string(
            template_name="user_home/product_list_partial.html", context=context
        )
        data = {
            "html": html,
...
def single_product_view(request, pk):
    ...
    all_variants = Product_Variant.objects.filter(
        product_name=product_variant.product_name
    )
    in_cart = CartItem.objects.filter(
        cart__cart_id=_cart_id(request), variant=product_variant
    ).exists()  # checking if the item is already added to the cart
    ...
    try:
        product_ordered = OrderProduct.objects.filter(
            order__user=request.user, product=product_variant
        ).exists()
    ...
    if request.method == "POST":
        try:
            reviews = ReviewRating.objects.get(
                user__id=request.user.id, product_variant__id=variant_id
            )
