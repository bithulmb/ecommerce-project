...
def _cart_id(request):
    ...
    )  # getting the session key and storing it as cartid
    if not cart:
        cart = (
            request.session.create()
        )  # if no session , creating a a session and storing it
    return cart
        ...
        try:
            ...
            if cart_item.quantity < variant.stock:
                if cart_item.quantity < 3:
                    ...
                    messages.error(
                        request, "Maximum 3 quantity per product is allowed for a user"
                    )
            ...
            cart_item = CartItem.objects.create(
                variant=variant, quantity=1, user=request.user
            )
            cart_item.save()
    else:
