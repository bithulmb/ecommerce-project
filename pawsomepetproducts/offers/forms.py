...
class AddProductOffer(forms.ModelForm):
    ...
    def clean_discount_amount(self):
        ...
        if product and discount_amount > product.price:
            raise ValidationError("Discount amount cannot be larger than the product price.")
...
class AddCategoryOffer(forms.ModelForm):
    ...
    def clean_discount_amount(self):
        ...
        if category:
            product_variants = Product_Variant.objects.filter(product_name__category=category)
            min_price = product_variants.aggregate(min_price=Min('price'))['min_price']
            
            if min_price is not None and discount_amount >= min_price:
                raise ValidationError(f"The discount amount should be less than the lowest product price (₹ {min_price}) in this category.")
